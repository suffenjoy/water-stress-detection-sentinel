...
- Also test two diffevalues for the cell a point falls in are returned. If 'bilinear' the returned values are interpolated from the values of the four nearest raster cells.rent extract methods: simple and bilinear. If 'simple' 
```{r}
...
```{r}
for(i in 1:length(swp_roop_20m)){
  swp_roop_20m[[i]]$vpd <- RHtoVPD(RH = swp_roop_20m[[i]]$humid, TdegC = (swp_roop_20m[[i]]$temp-32)*5/9)
}
...
write.csv(roop_2017_20m, file = file.path(path_output, "roop_2017_20m.csv"), row.names = FALSE)
